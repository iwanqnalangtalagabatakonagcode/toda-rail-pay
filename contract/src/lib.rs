...
impl TodaRailPay {
    ...
    pub fn open_route_pool(env: Env, id: String, owner: Address, target: i128) -> Result<PaymentRecord, ContractError> {
        if target <= 0 {
    ...
    pub fn collect_driver_due(env: Env, id: String, from: Address, amount: i128) -> Result<i128, ContractError> {
        if amount <= 0 {
            return Err(ContractError::InvalidAmount);
        }
        from.require_auth();
    ...
    pub fn approve_member_case(env: Env, id: String, score: u32, status: String) -> Result<PaymentRecord, ContractError> {
    ...
    pub fn release_member_fund(env: Env, id: String, to: Address, amount: i128) -> Result<i128, ContractError> {
        if amount <= 0 {
            return Err(ContractError::InvalidAmount);
        }
        Self::require_admin(&env);
...
mod test {
    ...
    fn constructor_sets_project_name() {
        ...
        let name = String::from_str(&env, "TODA Rail Pay");
        let contract_id = env.register(TodaRailPay, (admin, asset, name.clone()));
        let client = TodaRailPayClient::new(&env, &contract_id);
    ...
    fn creates_domain_record() {
        ...
        let asset = Address::generate(&env);
        let name = String::from_str(&env, "TODA Rail Pay");
        let contract_id = env.register(TodaRailPay, (admin, asset, name));
        let client = TodaRailPayClient::new(&env, &contract_id);

        let owner = Address::generate(&env);
        let id = String::from_str(&env, "record-001");
        let record = client.open_route_pool(&id, &owner, &1000);
